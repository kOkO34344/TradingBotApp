...
export default function ChartsPage() {
  ...
  // Restore the last chart setup — a charting screen that forgets what you
  // were looking at every reload is not one you'll keep open.
  useEffect(() => {
    try {
      ...
      if (Array.isArray(parsed.indicators)) setIndicators(parsed.indicators);
      if (typeof parsed.showLevels === "boolean") setShowLevels(parsed.showLevels);
    } catch {
      /* a corrupt preference is not worth surfacing */
    }
  }, []);
  ...
  }, [symbol, timeframe, indicators, showLevels]);
  ...
  const held: Position | undefined = positions.data?.positions.find(
    (p) => p.symbol === bars.data?.label
  );

  const lastBar = bars.data?.bars.at(-1);
  const prevBar = bars.data?.bars.at(-2);
  const change =
  ...
  return (
    <div className="flex h-[calc(100vh-3.5rem)] flex-col">
      ...
      <div className="flex flex-wrap items-baseline gap-x-6 gap-y-1 border-b border-border px-4 py-3">
        ...
        <div className="flex items-baseline gap-2">
          <span className="text-2xl font-semibold tabular-nums">
            {lastBar ? fmtPrice(lastBar.close, bars.data?.kind) : DASH}
          </span>
        ...
        {bars.data && (
          <div className="ml-auto flex items-center gap-3 text-xs text-muted-foreground">
            <span className="flex items-center gap-1">
              ...
              {bars.data.count} bars · {bars.data.duration}
            </span>
          </div>
        )}
      </div>
      ...
      <div className="relative flex-1 min-h-0 px-2 py-2">
        ...
        {!bars.error && bars.loading && !bars.data && (
        ...
        {bars.data && (
          <PriceChart data={bars.data} positions={positions.data?.positions} />
        )}
      </div>
      ...
      {bars.data && (
        <div className="flex flex-wrap items-center gap-x-4 gap-y-1 border-t border-border px-4 py-2 text-xs text-muted-foreground">
          <span>
            Source <span className="text-foreground">{bars.data.source}</span>
          </span>
          {bars.data.delayed && (
            <Badge
              variant="outline"
          ...
          <span>
            Fetched {fmtAge(bars.data.ageSeconds)}
            {bars.data.fromCache && " (cached)"}
          </span>
          <span>
            Last bar{" "}
          ...
          {bars.data.markers.length > 0 && (
            <span>
              {bars.data.markers.length} journal marker
              {bars.data.markers.length === 1 ? "" : "s"}
            </span>
          )}
        </div>
      )}
